...
export class StorageService {
  ...
  async download(urls: string[]) {
    ...
    await Promise.all(
      urls.map(async (download, idx) => {
        const singedUrl = await this.getSignedUrl(download);

        const data = await axios.get(singedUrl, {
          responseType: 'stream',
        });

        archive.append(data.data, { name: download.split('/').at(-1) });
      }),
    );
    return archive;
  }
